...
public class ConnectionPoolManager {
	...
	private PooledConnection connectionInTransition; // a PooledConnection which is currently within a
														// PooledConnection.getConnection() call, or null

	/**
	 * Constructs a ConnectionPoolManager object with a timeout of 60 seconds.
	 *
	 * @param dataSource
	...
	/**
	 * Constructs a ConnectionPoolManager object.
	 *
	 * @param dataSource
	 *            the data source for the connections.
	 * @param maxConnections
	...
	public Connection getConnection() throws SQLException {
		return getConnection2(timeoutMs, null, null);
	}

	public Connection getConnection(String username, String password) throws SQLException {
		return getConnection2(timeoutMs, username, password);
	}

	private Connection getConnection2(long timeoutMs, String username, String password) throws SQLException {
		...
		try {
			if (!semaphore.tryAcquire(timeoutMs, TimeUnit.MILLISECONDS)) {
				throw new TimeoutException("Can't acquire connection in " + timeoutMs + "ms");
			}
		...
		boolean ok = false;
		try {
			Connection conn = getConnection3(username, password);
			ok = true;
			return conn;
		} finally {
			if (!ok) {
	...
	private synchronized Connection getConnection3(String username, String password) throws SQLException {
		...
		if (!recycledConnections.isEmpty()) {
			...
			pconn = dataSource.getPooledConnection(username, password);
	...
	private Connection getValidConnection2(long time, long timeoutTime) {
		...
		Connection conn;
		try {
			conn = getConnection2(rtime, null, null);
		} catch (SQLException e) {
			return null;
		}
		...
		// This Exception should never occur. If it nevertheless occurs, it's because of
		// an error in the
		...
		// But to be sure that this works even with a faulty JDBC driver, we call
		// purgeConnection().
		purgeConnection(conn);
		return null;
	}

	// Purges the PooledConnection associated with the passed Connection from the
	// connection pool.
	private synchronized void purgeConnection(Connection conn) {
		try {
			...
			// has an effect only if the JDBC driver calls connectionClosed() synchronously
			// within
			// Connection.close().)
			conn.close();
		} catch (SQLException e) {
	...
	private synchronized void recycleConnection(PooledConnection pconn) {
		...
		if (pconn == connectionInTransition) {
			// This happens when a faulty JDBC driver calls
	...
	private void log(String msg) {
		String s = "ConnectionPoolManager: " + msg;
		try {
			if (logWriter == null) {
...
} // end class ConnectionPoolManager
